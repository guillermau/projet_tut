...
class Projet extends CI_Controller {
	...
	//cr�ation d'un projet par l'utilisateur
	 public function creer() {
	    $this->load->model("upload_model");
		
		$utilisateur = $this->profil_model->recuperer_mes_donnees(); //utilisateur r�cup�r� des cookies de la session
		
		//l'utilisateur ne peut cr�er un nouveau projet que s'il n'est pas invit�
		if( $utilisateur->invite == false){
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				if(empty($id)){
					$data = array('erreur' => 'Erreur dans la cr�ation du projet.');
				...
				//si tout a bien march�, renvoie a l'accueil du formularie
				redirect("mes-projets");
			}else{
			...
			//si j'ai pas les droits pour �tre ici
			$this->session->set_flashdata('erreur', 'Droits insufisants pour cr�er un projet.');
			redirect("mes-projets");
		}
	}
	...
	public function accueil($idprojet){
		...
		$data["projet"]      = $this->projet_model->recuperer_projet($idprojet);//r�cup�ration des infos du projet
		$data["nbdocs"]      = $this->projet_model->nombre_documents_prj($idprojet);//r�cup�ration du nombre de documents du projet
		$data["nbmembres"]   = $this->projet_model->nombre_membres_prj($idprojet);//r�cup�ration du nombre de membres du projet
		$data["nbinvites"]   = $this->projet_model->nombre_invites_prj($idprojet);//r�cup�ration du nombre de invit�s du projet
	...
	//page d'aper�u du projet pour un non membre
	public function apercu($idprojet){
		...
		$this->template->render('projet/apercu',$data);//appel au view aper�u du projet
	...
	public function documents($idprojet, $idrep = null){
		...
		//v�rifie si l'utilisateur � le droit d'acceder au projet
		...
		$data["idrep"]    = $idrep;
                //$data["arborescence"] = $this->repertoire_model->arborescence($idrep,$idprojet);
	...
	public function gestion($idprojet){
		//v�rifie si l'utilisateur est dans le groupe admin du projet
		$data["utilisateur"] = $this->profil_model->recuperer_mes_donnees();
	...
	public function modifier_image($idprojet){
		...
		// Si une image � �t� envoy�e, une array est retourn�e (m�me en cas d'echec)
		if(is_array($image)) {
	...
	//cr�ation d'un groupe dans le projet
	public function creer_groupe($idprojet){
		...
		$this->form_validation->set_rules('ecriture', 'Droit d\'�criture', 'required');
	...
	public function modifier_groupe($idprojet){
		...
		if ($this->form_validation->run() == TRUE){	
			...
			$this->projet_model->modifier_droits_gr($this->input->post("idgroupe",TRUE),
													 ...
													 $upload);
			echo "succes";
		//si formulaire non envoy� ou mal rempli
		}else{
			echo validation_errors();
		}
	}
	...
	public function supprimer_membre($idprojet, $idutilisateur){
		...
		$data["utilisateur"] = $this->profil_model->recuperer_mes_donnees();
		if(! $this->utilisateur_model->verifier_admin_projet($idprojet,$data["utilisateur"]->idutilisateur)) {
			echo "Vous n'�tes pas admin de ce projet.";
			return false;
		}
	...
	//Fonctions de callback de v�rification de formulaire

	public function verifie_groupe($idgroupe, $idprojet)
	{
		//v�rifie si le groupe existe et appartient au projet
	...
	public function creer_repertoire() {
		...
		// On d�finit les r�gle du formulaire
		$this->load->library('form_validation');
		
		$this->form_validation->set_rules('idprojet', 'projet', 'required');
		$this->form_validation->set_rules('nom', 'nom du r�pertoire', 'required');

		// Si le formulaire a �t� correctement envoy�
		if ($this->form_validation->run() == TRUE) {	

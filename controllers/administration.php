...
class Administration extends CI_Controller {
 ...
 //  TEST JUSTIFICATION                $data["justification"]= $this->utilisateur_model->recuperer_justification_utilisateur();
	...
	/////////////////////////////////////////
	// Syst�me de gestion des inscriptions //
	/////////////////////////////////////////

	// Accepter membre
	public function accepter_membre($idutilisateur) {
		// V�rification id fournis
		...
		// V�rification utilisateur en attente
		...
		// Si non, on retourne un fail
			...
			echo 'L\'utilisateur n\'est pas bloqu�!';
			return false;
		}
	}
	...
	public function modifier_utilisateur($idutilisateur) {
		if(!is_null($idutilisateur)) {
			...
			if ($this->form_validation->run() == TRUE){	
				...
				//$this->session->set_flashdata("succes","Donn�es modifi�es.");
	...
	// Envoyer message � tous les membres
	public function contacter_utilisateurs($destCat = "all", $id = null) {
		...
		// On v�rifie que les param�tres sont fournis
		if (empty($destCat) OR empty($objet) OR empty($message)) {
			echo "Param�tres incomplets";
		}
